...
#
# Security:
#   - NO external script downloads (removed gist dependency)
#   - Uses only local git commands
...
usage() {
  cat >&2 << 'USAGE'
Usage: generate-kui-changelog.sh -r <owner/repo> -b <base_sha> -t <target_sha> [-d <repo_dir>] -o <out_file>
...
Example:
  ./generate-kui-changelog.sh -r konflux-ci/konflux-ui -b abc123... -t def456... -o changelog.md
USAGE
}

# Validate SHA format (7-40 hex characters, supporting short SHAs)
validate_sha() {
  local sha="$1"
  local name="$2"
...
# Sanitize string for markdown (escape special characters)
sanitize_for_markdown() {
  local input="$1"
  # Remove control characters and escape backticks
...
# Validate output path is safe (no directory traversal)
if [[ "$OUT_FILE" == *".."* ]]; then
...
if ! git cat-file -t "$BASE_SHA" >/dev/null 2>&1; then
  log_warn "Base SHA not found locally, attempting deeper fetch..."
  git fetch --unshallow 2>/dev/null || git fetch --depth=1000 2>/dev/null || true
fi

if ! git cat-file -t "$TARGET_SHA" >/dev/null 2>&1; then
  log_warn "Target SHA not found locally, attempting deeper fetch..."
  git fetch --unshallow 2>/dev/null || git fetch --depth=1000 2>/dev/null || true
fi
